...
dev.copy2eps(file="meta_funnelplot_metafor.eps",width=7)


# Forestplot
dev.off()
par(mar=c(5,4,0,2))
#add to forest command to adjust ordering: order=order(data$r_adj)
forest(res, order=order(data$r_adj), xlim=c(-2.6,2.6),alim=c(-1,1), transf=transf.ztor, 
...
dev.print(postscript,"meta_forestplot_metafor.eps", width = 7, horizontal=FALSE)